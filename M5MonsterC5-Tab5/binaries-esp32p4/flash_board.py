...
REQUIRED_FILES = ["bootloader.bin", "partition-table.bin", "M5MonsterC5-Tab5.bin"]

# >>> DO NOT CHANGE: keep your original offsets <<<
OFFSETS = {
    "bootloader.bin": "0x2000",
    "partition-table.bin": "0x8000",
    "M5MonsterC5-Tab5.bin": "0x10000",
}
...
def erase_all(port, baud=DEFAULT_BAUD):
    cmd = [sys.executable, "-m", "esptool", "-p", port, "-b", str(baud),
           "--before", "default-reset", "--after", "no_reset", "--chip", "esp32p4",
           "erase_flash"]
...
def do_flash(port, baud=DEFAULT_BAUD, flash_mode="dio", flash_freq="80m"):
    cmd = [
        ...
        OFFSETS["M5MonsterC5-Tab5.bin"], "M5MonsterC5-Tab5.bin",
    ]
...
def main():
    parser = argparse.ArgumentParser(description="ESP32-P4 flasher with robust reboot handling")
    ...
    print(f"{CYAN}ESP32-P4 flasher version: {VERSION}{RESET}")
